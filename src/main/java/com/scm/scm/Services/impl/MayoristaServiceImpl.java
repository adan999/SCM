...
public class MayoristaServiceImpl implements MayoristaService {
    ...
    public boolean modificarMayorista(Mayorista mayorista) {
        try {
            String sql = String.format("UPDATE mayorista SET rfc = '" + mayorista.getRfc() + "'," +
                    "nomEmpresa = '" +mayorista.getNomEmpresa()+"', dir = '" +mayorista.getDir()+
                    "', numTel = '"+mayorista.getNumTel()+"' WHERE idMayorista = '" + mayorista.getIdMayorista() + "'");
            jdbcTemplate.execute(sql);
            return true;
        } catch (Exception e) {
            System.out.println(e.getMessage());
